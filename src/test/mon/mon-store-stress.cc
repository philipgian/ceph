              ...
              long long *num_ops)
{
  ...
  assert((e - s) < 2); // cap tx time at 2 seconds
}
...
int main(int argc, const char *argv[])
{
  ...
  for (std::vector<const char*>::iterator p = args.begin();
    ...
    if (ceph_argparse_double_dash(args, p)) {
      ...
      seed = (uint64_t) strict_strtoll(val.c_str(), 10, &err);
               ...
               "--test-num-keys", (char*)NULL)) {
      num_keys = (uint64_t) strict_strtoll(val.c_str(), 10, &err);
  ...
  if (write_min_size == 0) {
    write_min_size = 40960;
  }
  if (write_max_size == 0) {
    write_max_size = 512000;
  }
  if (write_max_size < write_min_size) {
    std::cerr << "error: 'write_max_size' < 'write_min_size'" << std::endl;
    return EINVAL;
  }
  ...
  while (true) {
    ...
    if (percent_write > 0 && p > (100-percent_write)) { // write
      ...
      std::cout << "write: prefix = " << prefix << ", key = " << v
                << ", size = " << s << std::endl;
      apply_tx(&db, &t, &num_ops);
      apply_tx(&db, &paxos_tx, &num_ops);
    ...
    if (ceph_clock_now(NULL) - last_trim > trim_period) {
      for (vector<string>::iterator p = prefixes.begin();
           p != prefixes.end(); ++p) {
        trim(db, prefix_fc, prefix_lc, *p, trim_from_zero, &num_ops);
      }
      trim(db, prefix_fc, prefix_lc, paxos_str, trim_from_zero, &num_ops);
